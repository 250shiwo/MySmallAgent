...
## 更新摘要
**所做更改**
- 更新 WebSearch 工具依赖库从 duckduckgo-search 迁移到 ddgs 库
- 更新 CurrentTime 工具的时区处理机制
- 更新测试文件中的导入语句和断言
- 修正工具注册表中的工具列表描述
...
M[prompt_toolkit 模块]
N[rich 模块]
O[ddgs 模块] <!-- 更新: 从 duckduckgo_search 改为 ddgs -->
P[datetime 模块]
Q[zoneinfo 模块]
R[tzdata 模块]
end
...
D --> J
E --> K
F --> O <!-- 更新: 从 duckduckgo_search 改为 ddgs -->
G --> P
G --> Q
H --> M
H --> N
...
**重要更新**: WebSearch 工具已从废弃的 duckduckgo-search 库迁移到活跃维护的 ddgs 包。这次迁移保持了 API 兼容性，同时提供了更好的维护性和稳定性：

- **库名变更**: `duckduckgo_search` → `ddgs`
- **API 兼容**: `AsyncDDGS()` → `DDGS()`，保持相同的接口
- **版本要求**: 最低版本要求为 7.0 或更高
- **性能改进**: ddgs 库提供了更好的性能和稳定性
- **社区支持**: ddgs 是活跃维护的库，有更好的长期支持